...
\end{center}
    ...
    \begin{enumerate}
        \item First row and column: Since $1 \cdot a = a$ for any integer $a$, the first row and first column are identical to the header row and column.
        
        \item A Sample of Calculation: To determine non-trivial entries, we perform multiplication modulo 8. 
        For example, consider the entry for row 3 and column 5:
        \[ 3 \cdot 5 = 15 \]
        Dividing by 8, we get a remainder of 7 ($15 = 1(8) + 7$).
        \[ 15 \equiv 7 \pmod 8 \]
        Thus, the entry at $(3,5)$ is 7.
        
        \item Inverses We observe the main diagonal consists entirely of $1$s:
        \[ 3^2 = 9 \equiv 1, \quad 5^2 = 25 \equiv 1, \quad 7^2 = 49 \equiv 1 \pmod 8 \]
        This confirms that every element is its own inverse.
    \end{enumerate}
\end{solution}



\question[] All $n\times n$ diagonal matrices with diagonal entries does not form a group under matrix multiplication.

\begin{solution}
\textbf{General idea}
For a set of matrices to form a group under multiplication, every element must have a multiplicative inverse. However, a matrix is invertible if and only if its determinant is non-zero. The set of all diagonal matrices includes the zero matrix, which has a determinant of 0. does not have inverse and disprove the statement. But before that, we must first ensure the identity exists to proceed the proof.


    Let $D$ be the set of all $n \times n$ diagonal matrices with entries in $\mathbb{R}$.



    
    We claim that $D$ is not a group under matrix multiplication because it does not have inverse for each element.

    \begin{proof}

  First, note that the identity matrix $I_n$ is a diagonal matrix, so $I_n \in D$, and it acts as the multiplicative identity because by MAT215 we know that the identity matrix multiply with arbitrary matrix $A$ result $A$ it self. Since $D$ is a subset of $n\times n$ matrix this property also holds for $D$.


    Consider the zero matrix $\vec{0} \in D$, where all diagonal entries are 0.
    For $D$ to be a group, there must exist an inverse matrix $\vec{0}^{-1} \in D$ such that:
    \[ \vec{0} \cdot \vec{0}^{-1} = I_n \]
    where $I_n$ is the identity matrix.
    
    However, for any matrix $A \in D$, the product $\vec{0} \cdot A$ is:
    \[ \vec{0} \cdot A = \vec{0} \]
    Since $\vec{0} \neq I_n$, it is impossible to find an element $A$ such that $\vec{0} \cdot A = I_n$.
    
    Thus, the element $\vec{0}$ has no inverse. Since not all elements in $D$ are invertible, $D$ is not a group.
    \end{proof}
\end{solution}
...
\begin{equation}
(a*b)*(a*b) = e \label{eq:self_inverse}
\end{equation}
...
I collaborate with sophia on this assignment where we went through all these questions and she accidentally ignore the word "diagonal" in question 2 and I reminded her about that and showed her that how to prove the closure for the matrix multiplication on that question. And she asked me whether I should also put a full proof for the associativity for matrix multiplication and I responses that since we already know that associativity holds for all n by n matrices it must also hold for diagonal matrix but I think she indeed point out one of the important thing in whether its true that a binary operation is associative for each element then it's associative on a specific type of elements.
