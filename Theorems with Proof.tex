...
Let $G$ be a group and let $x\in G$. Define the \emph{left multiplication map}
\[
\lambda_x : G \to G,\qquad \lambda_x(g)=xg.
\]
...
\subsubsection{Lemma 8B}
\begin{lemma} 8B
For any $x,y\in G$ we have $\lambda_x\circ \lambda_y=\lambda_{xy}$.
Moreover, $\lambda_e$ is the identity function on $G$.
\end{lemma}

\begin{proof}
Let $g\in G$ be arbitrary. \proofnote{To show two functions are equal, test on an arbitrary input.}
By definition of composition,
\[
(\lambda_x\circ \lambda_y)(g)=\lambda_x(\lambda_y(g)).
\]
Using the definition of $\lambda_y$ and then $\lambda_x$,
\[
\lambda_x(\lambda_y(g))=\lambda_x(yg)=x(yg).
\]
On the other hand,
\[
\lambda_{xy}(g)=(xy)g.
\]
By associativity of the group operation, $x(yg)=(xy)g$, so
$\lambda_x\circ \lambda_y=\lambda_{xy}$.

For the second statement, for any $g\in G$,
\[
\lambda_e(g)=eg=g,
\]
so $\lambda_e$ fixes every element of $G$, i.e. it is the identity function.
\end{proof}

\subsubsection{Lemma 8C}

\begin{lemma} 8C
For any $x\in G$, the map $\lambda_x$ is a permutation of $G$; equivalently,
$\lambda_x\in S_G$.
\end{lemma}

\begin{proof}
We show $\lambda_x$ is a bijection by exhibiting its inverse. \proofnote{Bijective $\Leftrightarrow$ has an inverse.}
Consider $\lambda_{x^{-1}}:G\to G$. By Lemma 8B,
\[
\lambda_x\circ \lambda_{x^{-1}}=\lambda_{xx^{-1}}=\lambda_e,
\]
which is the identity on $G$. Similarly,
\[
\lambda_{x^{-1}}\circ \lambda_x=\lambda_{x^{-1}x}=\lambda_e.
\]
Thus $\lambda_{x^{-1}}$ is the inverse of $\lambda_x$, so $\lambda_x$ is a bijection,
hence a permutation of $G$. Therefore $\lambda_x\in S_G$.
\end{proof}
...
\begin{proof}
For each $x\in G$, Lemma 8C gives $\lambda_x\in S_G$. Define
\[
\varphi:G\to S_G,\qquad \varphi(x)=\lambda_x.
\]
We claim $\varphi$ is a homomorphism. Let $x,y\in G$ be arbitrary. By Lemma 8B,
\[
\varphi(x)\circ \varphi(y)=\lambda_x\circ \lambda_y=\lambda_{xy}=\varphi(xy).
\]
\proofnote{Key step: Lemma 8B turns composition into multiplication.}

Next we show $\varphi$ is injective. Suppose $\varphi(x)=\varphi(y)$, i.e.
$\lambda_x=\lambda_y$. Then in particular they agree on the identity element $e$:
\[
\lambda_x(e)=\lambda_y(e).
\]
So $xe=ye$, hence $x=y$. \proofnote{Evaluate at $e$ to recover the element.}

Therefore $\varphi$ is an injective homomorphism $G\to S_G$, so $G$ is isomorphic
to its image $\varphi(G)$, and $\varphi(G)$ is a subgroup of $S_G$.
\end{proof}
